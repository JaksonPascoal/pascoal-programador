...
class CompliantSelector(CompliantExpr[FrameT, SeriesOrExprT], Protocol[FrameT, SeriesOrExprT]):
    ...
    def __sub__(
        ...
        if self._is_selector(other):
            ...
            def series(df: FrameT) -> Sequence[SeriesOrExprT]:
                ...
                return [x for x, name in zip_strict(self(df), lhs_names) if name not in rhs_names]
    ...
    def __or__(
        ...
        if self._is_selector(other):
            ...
            def series(df: FrameT) -> Sequence[SeriesOrExprT]:
                lhs_names, rhs_names = _eval_lhs_rhs(df, self, other)
                return [
                    *(x for x, name in zip_strict(self(df), lhs_names) if name not in rhs_names),
                    *other(df),
                ]
    ...
    def __and__(
        ...
        if self._is_selector(other):
            ...
            def series(df: FrameT) -> Sequence[SeriesOrExprT]:
                lhs_names, rhs_names = _eval_lhs_rhs(df, self, other)
                return [x for x, name in zip_strict(self(df), lhs_names) if name in rhs_names]
