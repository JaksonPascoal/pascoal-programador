...
class DaskExpr(
    ...
    def _with_binary(
        ...
        *,
        ...
        result = self._with_callable(lambda expr, other: call(expr, other), name, other=other)
        if reverse:
            result = result.alias("literal")
        return result

    def _binary_op(self, op_name: str, other: Any) -> Self:
        return self._with_binary(lambda expr, other: getattr(expr, op_name)(other), op_name, other)
    ...
    def cum_count(self, *, reverse: bool) -> Self:
        ...
        return self._with_callable(lambda expr: (~expr.isna()).astype(int).cumsum(), "cum_count")
    ...
    def all(self) -> Self:
        return self._with_callable(
            lambda expr: expr.all(axis=None, skipna=True, split_every=False, out=None).to_series(),
            "all",
        )
    ...
    def fill_null(
        ...
        def func(expr: dx.Series) -> dx.Series:
            if value is not None:
                ...
                res_ser = (
                    expr.ffill(limit=limit) if strategy == "forward" else expr.bfill(limit=limit)
                )
            return res_ser
    ...
    def clip(
        ...
        return self._with_callable(
            lambda expr, lower_bound, upper_bound: expr.clip(lower=lower_bound, upper=upper_bound),
            "clip",
    ...
    def n_unique(self) -> Self:
        return self._with_callable(lambda expr: expr.nunique(dropna=False).to_series(), "n_unique")
    ...
    def is_nan(self) -> Self:
        def func(expr: dx.Series) -> dx.Series:
            dtype = native_to_narwhals_dtype(expr.dtype, self._version, self._implementation)
            if dtype.is_numeric():
        ...
        if interpolation == "linear":
            ...
            def func(expr: dx.Series, quantile: float) -> dx.Series:
                if expr.npartitions > 1:
                    ...
                    raise NotImplementedError(msg)
                return expr.quantile(q=quantile, method="dask").to_series()  # pragma: no cover
    ...
    def is_unique(self) -> Self:
        def func(expr: dx.Series) -> dx.Series:
            _name = expr.name
            return (
                expr.to_frame().groupby(_name, dropna=False).transform("size", meta=(_name, int))
                == 1
            )
    ...
    def null_count(self) -> Self:
        return self._with_callable(lambda expr: expr.isna().sum().to_series(), "null_count")
