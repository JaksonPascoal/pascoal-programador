...
class DaskNamespace(
        ...
        if how == "vertical":
            for i, df in enumerate(dfs[1:], start=1):
                ...
                if not ((len(cols_current) == len(cols_0)) and (cols_current == cols_0).all()):
                    ...
                    raise TypeError(msg)
            return DaskLazyFrame(dd.concat(dfs, axis=0, join="inner"), version=self._version)
        if how == "diagonal":
            return DaskLazyFrame(dd.concat(dfs, axis=0, join="outer"), version=self._version)
    ...
    def concat_str(self, *exprs: DaskExpr, separator: str, ignore_nulls: bool) -> DaskExpr:
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            expr_results = [s for _expr in exprs for s in _expr(df)]
            ...
            if not ignore_nulls:
                ...
                init_value, *values = [s.where(~nm, "") for s, nm in zip_strict(series, null_mask)]
...
class DaskWhen(
    ...
    def __call__(self, df: DaskLazyFrame) -> Sequence[dx.Series]:
        then_value = (
            self._then_value(df)[0] if isinstance(self._then_value, DaskExpr) else self._then_value
        )
        otherwise_value = (
        ...
        if self._otherwise_value is None:
            (condition, then_series) = align_series_full_broadcast(df, condition, then_value)
