...
class DuckDBLazyFrame(
    ...
    @classmethod
    def from_native(cls, data: duckdb.DuckDBPyRelation, /, *, context: _LimitedContext) -> Self:
    ...
    def schema(self) -> dict[str, DType]:
        ...
        return {
            column_name: native_to_narwhals_dtype(duckdb_dtype, self._version, deferred_time_zone)
            for column_name, duckdb_dtype in zip_strict(self.native.columns, self.native.types)
        }
    ...
    def columns(self) -> list[str]:
        if self._cached_columns is None:
            self._cached_columns = (
                list(self.schema) if self._cached_native_schema is not None else self.native.columns
            )
    ...
    def unique(self, subset: Sequence[str] | None, *, keep: LazyUniqueKeepStrategy) -> Self:
        if subset_ := subset if keep == "any" else (subset or self.columns):
            ...
            count_expr = window_expression(F("count", StarExpression()), subset_, ()).alias(
                count_name
            )
    ...
    def explode(self, columns: Sequence[str]) -> Self:
        ...
        not_null_condition = col_to_explode.isnotnull() & F("len", col_to_explode) > lit(0)
