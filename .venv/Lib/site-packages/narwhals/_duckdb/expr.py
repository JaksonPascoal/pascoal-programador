...
class DuckDBExpr(SQLExpr["DuckDBLazyFrame", "Expression"]):
    ...
    def skew(self) -> Self:
        def func(expr: Expression) -> Expression:
            ...
            sample_skewness = (
                F("skewness", expr) * (count - lit(2)) / F("sqrt", count * (count - lit(1)))
            )
    ...
    def std(self, ddof: int) -> Self:
        ...
        def _std(expr: Expression) -> Expression:
            ...
            return (
                F("stddev_pop", expr) * F("sqrt", n_samples) / (F("sqrt", (n_samples - lit(ddof))))
            )
    ...
    def fill_null(
        ...
        if strategy is not None:
            if self._backend_version < (1, 3):  # pragma: no cover
                msg = (
                    f"`fill_null` with `strategy={strategy}` is only available in 'duckdb>=1.3.0'."
                )
                raise NotImplementedError(msg)
