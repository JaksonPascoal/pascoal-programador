    ...
    if exclude:
        ...
        if expr._metadata.expansion_kind.is_multi_unnamed():
            output_names, aliases = zip_strict(
                *[(x, alias) for x, alias in zip_strict(output_names, aliases) if x not in exclude]
            )
...
def check_expressions_preserve_length(*args: IntoExpr, function_name: str) -> None:
    ...
    if not all(
        (is_expr(x) and x._metadata.preserves_length) or isinstance(x, (str, Series)) for x in args
    ):
