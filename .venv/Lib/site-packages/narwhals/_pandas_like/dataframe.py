...
class PandasLikeDataFrame(
    ...
    def with_row_index(self, name: str, order_by: Sequence[str] | None) -> Self:
        ...
        if order_by is None:
            ...
            row_index = plx._expr._from_series(
                plx._series.from_iterable(data, context=self, index=self.native.index, name=name)
            )
        else:
    ...
    def _join_semi(
        ...
        return self.native.merge(other_native, how="inner", left_on=left_on, right_on=left_on)
    ...
    def join(
        ...
        *,
        ...
        if how == "cross":
            ...
            result = self._join_left(other=other, left_on=left_on, right_on=right_on, suffix=suffix)
        elif how == "full":
            result = self._join_full(other=other, left_on=left_on, right_on=right_on, suffix=suffix)
        else:
            assert_never(how)
    ...
    def _pivot_into_index_values(
        ...
        index = index or (
            exclude_column_names(self, {*on, *values}) if values else exclude_column_names(self, on)
        )
    ...
    def _pivot_table(
        ...
        index: Sequence[str],
        values: Sequence[str],
        aggregate_function: Literal["min", "max", "first", "last", "sum", "mean", "median"],
        /,
    ) -> Any:
        kwds: dict[Any, Any] = (
    ...
    def pivot(
        ...
        *,
        ...
        uniques = (
            (
                self.get_column(col).unique().sort(descending=False, nulls_last=False).to_list()
                for col in on
            )
            if sort_columns
    ...
    def explode(self, columns: Sequence[str]) -> Self:
        ...
        if not all(
            (native_frame[col_name].list.len() == anchor_series).all() for col_name in columns[1:]
        ):
        ...
        exploded_series = [native_frame[col_name].explode().to_frame() for col_name in columns[1:]]
