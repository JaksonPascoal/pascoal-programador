...
class PandasLikeExpr(EagerExpr["PandasLikeDataFrame", PandasLikeSeries]):
    ...
    def from_column_indices(cls, *column_indices: int, context: _LimitedContext) -> Self:
        def func(df: PandasLikeDataFrame) -> list[PandasLikeSeries]:
            native = df.native
            return [
                PandasLikeSeries.from_native(native.iloc[:, i], context=df) for i in column_indices
            ]
    ...
    def over(  # noqa: C901, PLR0915
        ...
        if not partition_by:
            ...
            if pandas_function_name is None:
                ...
                raise NotImplementedError(msg)
            pandas_kwargs = window_kwargs_to_pandas_equivalent(function_name, self._scalar_kwargs)
