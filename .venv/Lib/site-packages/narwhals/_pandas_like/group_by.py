...
class AggExpr:
    ...
    def with_expand_names(self, group_by: PandasLikeGroupBy, /) -> AggExpr:
        ...
        self.output_names, self.aliases = evaluate_output_names_and_aliases(self.expr, df, exclude)
        return self
    ...
    def native_agg(self) -> _NativeAgg:
        ...
        return _native_agg(PandasLikeGroupBy._remap_expr_name(self.leaf_name), **self.kwargs)


class PandasLikeGroupBy(EagerGroupBy["PandasLikeDataFrame", "PandasLikeExpr", NativeAggregation]):
    ...
    def __init__(
        ...
        *,
        ...
        self._compliant_frame, self._keys, self._output_key_names = self._parse_keys(df, keys)
    ...
    def _getitem_aggs(self, exprs: Iterable[AggExpr], /) -> list[pd.DataFrame | pd.Series[Any]]:
    ...
    def _apply_exprs_function(self, exprs: Iterable[PandasLikeExpr]) -> NativeApply:
        ...
        def fn(df: pd.DataFrame) -> pd.Series[Any]:
            ...
            results = (
                (keys.native.iloc[0], keys.name) for expr in exprs for keys in expr(compliant)
            )
