...
class PandasLikeSeries(EagerSeries[Any]):
    ...
    def _with_version(self, version: Version) -> Self:
        return self.__class__(self.native, implementation=self._implementation, version=version)

    def _with_native(self, series: Any, *, preserve_broadcast: bool = False) -> Self:
        result = self.__class__(series, implementation=self._implementation, version=self._version)
    ...
    def _align_full_broadcast(cls, *series: Self) -> Sequence[Self]:
        ...
        for s in series:
            if s._broadcast:
                native = Series(s.native.iloc[0], index=idx, name=s.name, dtype=s.native.dtype)
    ...
    def fill_null(
        ...
        kwargs = (
            {"downcast": False}
            if self._implementation is Implementation.PANDAS and self._backend_version < (3,)
            else {}
        )
    ...
    def clip(
        ...
        _, lower = (
            align_and_extract_native(self, lower_bound) if lower_bound is not None else (None, None)
        )
        _, upper = (
            align_and_extract_native(self, upper_bound) if upper_bound is not None else (None, None)
        )
    ...
    def rank(self, method: RankMethod, *, descending: bool) -> Self:
        ...
        if (
            ...
            and get_dtype_backend(self.native.dtype, self._implementation) == "numpy_nullable"
    ...
    def hist_from_bins(self, bins: list[float], *, include_breakpoint: bool) -> PandasLikeDataFrame:
        return (
