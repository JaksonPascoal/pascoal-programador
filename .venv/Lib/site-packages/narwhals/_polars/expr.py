...
class PolarsExpr:
    ...
    def over(self, partition_by: Sequence[str], order_by: Sequence[str]) -> Self:
        if self._backend_version < (1, 9):
            ...
            native = self.native.over(partition_by or pl.lit(1), order_by=order_by or None)
        return self._with_native(native)
...
class PolarsExprStringNamespace(PolarsExprNamespace, PolarsStringNamespace[PolarsExpr, pl.Expr]):
    def zfill(self, width: int) -> PolarsExpr:
        ...
        if backend_version <= (1, 30, 0):
            ...
            native_result = (
                ...
                .then(
                    self.native.str.slice(1, length).str.zfill(width - 1).str.pad_start(width, plus)
                )
...
class PolarsExprCatNamespace(PolarsExprNamespace, PolarsCatNamespace[PolarsExpr, pl.Expr]): ...
...
class PolarsExprListNamespace(PolarsExprNamespace, PolarsListNamespace[PolarsExpr, pl.Expr]):
