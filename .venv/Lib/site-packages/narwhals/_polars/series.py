...
class PolarsSeries:
    ...
    def replace_strict(
        ...
        *,
        ...
        ser = self.native
        dtype = narwhals_to_native_dtype(return_dtype, self._version) if return_dtype else None
    ...
    def is_close(
        ...
        *,
        ...
        if self._backend_version < (1, 32, 0):
            ...
            other_expr = ns.lit(other.native, None) if isinstance(other, PolarsSeries) else other
    ...
    def hist_from_bins(self, bins: list[float], *, include_breakpoint: bool) -> PolarsDataFrame:
        if len(bins) <= 1:
    ...
    def hist_from_bin_count(self, bin_count: int, *, include_breakpoint: bool) -> PolarsDataFrame:
        if bin_count == 0:
            native = pl.DataFrame(schema=self._HIST_EMPTY_SCHEMA[include_breakpoint])
        elif self.native.is_empty():
            if include_breakpoint:
