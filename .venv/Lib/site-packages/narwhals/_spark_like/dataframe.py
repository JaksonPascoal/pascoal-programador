...
class SparkLikeLazyFrame(
    ...
    def __narwhals_namespace__(self) -> SparkLikeNamespace:
        ...
        return SparkLikeNamespace(version=self._version, implementation=self._implementation)

    def __narwhals_lazyframe__(self) -> Self:
        return self

    def _with_version(self, version: Version) -> Self:
        return self.__class__(self.native, version=version, implementation=self._implementation)

    def _with_native(self, df: SQLFrameDataFrame) -> Self:
        return self.__class__(df, version=self._version, implementation=self._implementation)
    ...
    def _collect(self, backend: _EagerAllowedImpl | None, **kwargs: Any) -> CompliantDataFrameAny:
    ...
    def top_k(self, k: int, *, by: Iterable[str], reverse: bool | Sequence[bool]) -> Self:
        ...
        sort_funcs = (self._F.desc_nulls_last if not d else self._F.asc_nulls_last for d in reverse)
        sort_cols = [sort_f(col) for col, sort_f in zip_strict(by, sort_funcs)]
    ...
    def with_row_index(self, name: str, order_by: Sequence[str]) -> Self:
        if order_by is None:
            ...
            raise TypeError(msg)
        row_index_expr = (
            self._F.row_number().over(self._Window.partitionBy(self._F.lit(1)).orderBy(*order_by))
            - 1
        ).alias(name)
