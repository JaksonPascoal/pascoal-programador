...
class SQLExpr(LazyExpr[SQLLazyFrameT, NativeExprT], Protocol[SQLLazyFrameT, NativeExprT]):
    ...
    def _callable_to_eval_series(
        ...
        def func(df: SQLLazyFrameT) -> list[NativeExprT]:
            ...
            other_native_series = {
                key: df._evaluate_expr(value) if self._is_expr(value) else self._lit(value)
    ...
    def _cum_window_func(
        ...
        *,
        ...
        def func(df: SQLLazyFrameT, inputs: WindowInputs[NativeExprT]) -> Sequence[NativeExprT]:
            return [
    ...
    def _rolling_window_func(
        ...
        *,
        ...
        def func(df: SQLLazyFrameT, inputs: WindowInputs[NativeExprT]) -> Sequence[NativeExprT]:
            ...
            return [
                self._when(
                    self._window_expression(self._function("count", expr), **window_kwargs)
    ...
    def _from_elementwise_horizontal_op(
        ...
        def window_function(
            ...
            cols = (col for _expr in exprs for col in _expr.window_function(df, window_inputs))
            return [func(cols)]
    ...
    def all(self) -> Self:
        ...
        def window_f(df: SQLLazyFrameT, inputs: WindowInputs[NativeExprT]) -> Sequence[NativeExprT]:
            return [
                self._coalesce(
                    self._window_expression(self._function("bool_and", expr), inputs.partition_by),
                    self._lit(True),
                )
                for expr in self(df)
            ]
    ...
    def any(self) -> Self:
        ...
        def window_f(df: SQLLazyFrameT, inputs: WindowInputs[NativeExprT]) -> Sequence[NativeExprT]:
            return [
                self._coalesce(
                    self._window_expression(self._function("bool_or", expr), inputs.partition_by),
                    self._lit(False),
                )
                for expr in self(df)
            ]
    ...
    def clip(
        ...
        def _clip_both(expr: NativeExprT, lower_bound: Any, upper_bound: Any) -> NativeExprT:
        ...
        return self._with_elementwise(_clip_both, lower_bound=lower_bound, upper_bound=upper_bound)
    ...
    def cum_prod(self, *, reverse: bool) -> Self:
        return self._with_window_function(self._cum_window_func("product", reverse=reverse))
    ...
    def rolling_var(self, window_size: int, *, min_samples: int, center: bool, ddof: int) -> Self:
        return self._with_window_function(
            self._rolling_window_func("var", window_size, min_samples, ddof=ddof, center=center)
        )

    def rolling_std(self, window_size: int, *, min_samples: int, center: bool, ddof: int) -> Self:
        return self._with_window_function(
            self._rolling_window_func("std", window_size, min_samples, ddof=ddof, center=center)
        )
    ...
    def rank(self, method: RankMethod, *, descending: bool) -> Self:
        ...
        return self._with_callable(_unpartitioned_rank)._with_window_function(_partitioned_rank)

    def is_unique(self) -> Self:
        def _is_unique(expr: NativeExprT, *partition_by: str | NativeExprT) -> NativeExprT:
            return self._window_expression(self._count_star(), (expr, *partition_by)) == self._lit(
                1
            )
    ...
    # Other
    def over(self, partition_by: Sequence[str | NativeExprT], order_by: Sequence[str]) -> Self:
