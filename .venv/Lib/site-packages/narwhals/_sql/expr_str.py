...
class SQLExprStringNamespace(
    ...
    def len_chars(self) -> SQLExprT:
        return self.compliant._with_elementwise(lambda expr: self._function("length", expr))

    def replace_all(self, pattern: str, value: str | SQLExprT, *, literal: bool) -> SQLExprT:
        ...
        return self.compliant._with_elementwise(
            lambda expr, value: self._function(fname, expr, self._lit(pattern), value, *options),
            value=value,
        )
    ...
    def slice(self, offset: int, length: int | None) -> SQLExprT:
        def func(expr: SQLExprT) -> SQLExprT:
            ...
            _offset = col_length + self._lit(offset + 1) if offset < 0 else self._lit(offset + 1)
    ...
    def to_lowercase(self) -> SQLExprT:
        return self.compliant._with_elementwise(lambda expr: self._function("lower", expr))

    def to_uppercase(self) -> SQLExprT:
        return self.compliant._with_elementwise(lambda expr: self._function("upper", expr))
    ...
    def zfill(self, width: int) -> SQLExprT:
        ...
        def func(expr: Any) -> Any:
            ...
            padded_substring = self._function("lpad", substring, self._lit(width - 1), zero)
            return self._when(
