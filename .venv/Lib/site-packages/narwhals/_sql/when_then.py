...
class SQLWhen(
    ...
    def _window_function(
        ...
        then_ = self._then_value
        then = then_.window_function(df, window_inputs)[0] if is_expr(then_) else lit(then_)

        other_ = self._otherwise_value
        if other_ is None:
            result = when(condition, then)
        else:
            other = other_.window_function(df, window_inputs)[0] if is_expr(other_) else lit(other_)
