...
class BaseFrame(Generic[_FrameT]):
    ...
    def top_k(
        ...
        return self._with_compliant(self._compliant_frame.top_k(k, by=flatten_by, reverse=reverse))
    ...
    def join(
        ...
        *,
        ...
        if how == "cross":
            ...
            result = compliant.join(other, how=how, left_on=None, right_on=None, suffix=suffix)
        elif on is None:
            ...
            if left_on is not None or right_on is not None:
                ...
                raise ValueError(msg)
            result = compliant.join(other, how=how, left_on=on, right_on=on, suffix=suffix)
...
class DataFrame(BaseFrame[DataFrameT]):
    ...
    def with_row_index(
        ...
        return self._with_compliant(self._compliant_frame.with_row_index(name, order_by=order_by_))
    ...
    def group_by(
        ...
        _keys = [
            k if is_expr else col(k) for k, is_expr in zip_strict(flat_keys, key_is_expr_or_series)
        ]
...
class LazyFrame(BaseFrame[LazyFrameT]):
    ...
    def with_row_index(self, name: str = "index", *, order_by: str | Sequence[str]) -> Self:
        """Insert column which enumerates rows.
    ...
    def group_by(
        ...
        _keys = [k if is_expr else col(k) for k, is_expr in zip_strict(flat_keys, key_is_expr)]
        expr_flat_keys, kinds = self._flatten_and_extract(*_keys)
