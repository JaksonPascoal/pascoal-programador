...
def show_versions():
    ...
    print_entry(
        "Package kind",
        cpp_build_info.package_kind if len(cpp_build_info.package_kind) > 0 else "not indicated",
    )
    print_entry("Arrow C++ library version", cpp_build_info.version)
    print_entry(
        "Arrow C++ compiler", f"{cpp_build_info.compiler_id} {cpp_build_info.compiler_version}"
    )
        ...
        if _has_pkg_config(pkgname):
            ...
            if library_dir:
                if not library_dir.startswith("-L"):
                    raise ValueError(
                        "pkg-config --libs-only-L returned unexpected " f"value {library_dir!r}"
                    )
