...
def partitioning(schema=None, field_names=None, flavor=None, dictionaries=None):
    ...
    if flavor == "filename":
        if schema is not None:
            ...
            raise ValueError(
                "For the filename flavor, need to specify " "a Schema or a list of field names"
            )
    elif flavor == "hive":
        if field_names is not None:
...
def _union_dataset(children, schema=None, **kwargs):
    if any(v is not None for v in kwargs.values()):
        raise ValueError(
            "When passing a list of Datasets, you cannot pass any additional " "arguments"
        )
...
def _ensure_write_partitioning(part, schema, flavor):
    ...
    if isinstance(part, Partitioning) and flavor:
        raise ValueError(
            "Providing a partitioning_flavor with " "a Partitioning object is not supported"
        )
