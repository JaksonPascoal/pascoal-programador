...
def get_column_metadata(column, name, arrow_type, field_name):
    ...
    if (
        ...
        raise TypeError(
            f"Column name must be a string. Got column {name} of type " f"{type(name).__name__}"
        )
...
def construct_metadata(
    ...
    for col, name, field_name, arrow_type in zip(
        ...
        metadata = get_column_metadata(col, name=name, arrow_type=arrow_type, field_name=field_name)
...
def _resolve_columns_of_interest(df, schema, columns):
    if schema is not None and columns is not None:
        raise ValueError(
            "Schema and columns arguments are mutually " "exclusive, pass only one of them"
        )
...
def dataframe_to_arrays(df, schema, preserve_index, nthreads=1, columns=None, safe=True):
    ...
    def convert_column(col, field):
        ...
        try:
            ...
            e.args += (f"Conversion failed for column {col.name} with type {col.dtype}",)
            raise e
...
def table_to_dataframe(options, table, categories=None, ignore_metadata=False, types_mapper=None):
    ...
    if not ignore_metadata and pandas_metadata is not None:
        ...
        ext_columns_dtypes = _get_extension_dtypes(table, [], types_mapper, options, categories)
    ...
    if _pandas_api.is_ge_v3():
        ...
        blocks = [
            _reconstruct_block(item, column_names, ext_columns_dtypes, return_block=False)
            for item in result
        ]
...
def _check_data_column_metadata_consistency(all_columns):
    ...
    assert all(
        (c["name"] is None and "field_name" in c) or c["name"] is not None for c in all_columns
    )
...
def _deserialize_column_index(block_table, all_columns, column_indexes):
    if all_columns:
        columns_name_dict = {
            c.get("field_name", _column_name_to_strings(c["name"])): c["name"] for c in all_columns
        }
