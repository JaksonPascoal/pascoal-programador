...
def test_byte_stream_split():
    ...
    _check_roundtrip(
        table, expected=table, compression="gzip", use_dictionary=False, use_byte_stream_split=True
    )

    # Check with byte_stream_split for column 'b' and dictionary
    # for column 'a'.
    _check_roundtrip(
        table, expected=table, compression="gzip", use_dictionary=["a"], use_byte_stream_split=["b"]
    )
...
def test_store_decimal_as_integer(tempdir):
    ...
    _check_roundtrip(
        mixed_table, expected=mixed_table, use_dictionary=False, store_decimal_as_integer=True
    )
...
def test_column_encoding():
    ...
    _check_roundtrip(
        mixed_table, expected=mixed_table, use_dictionary=False, column_encoding={"e": "RLE"}
    )
    ...
    with pytest.raises(ValueError):
        _check_roundtrip(
            mixed_table, expected=mixed_table, use_dictionary=["b"], column_encoding={"b": "PLAIN"}
        )
    ...
    with pytest.raises(TypeError):
        _check_roundtrip(
            mixed_table, expected=mixed_table, use_dictionary=False, column_encoding=True
        )
...
def test_thrift_size_limits(tempdir):
    ...
    num_cols = 1000
    table = pa.table(
        [array] * num_cols, names=[f"some_long_column_name_{i}" for i in range(num_cols)]
    )
    pq.write_table(table, path)
