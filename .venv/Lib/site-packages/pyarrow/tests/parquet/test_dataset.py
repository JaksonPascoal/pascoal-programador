...
def _partition_test_for_filesystem(fs, base_path):
    ...
    expected_df = (
        df.sort_values(by="index").reset_index(drop=True).reindex(columns=result_df.columns)
    )
...
@pytest.mark.skipif(sys.platform == "win32", reason="test fails because of unsupported characters")
...
def _make_dataset_for_pickling(tempdir, N=100):
    ...
    df = pd.DataFrame(
        {"index": np.arange(N), "values": np.random.randn(N)}, columns=["index", "values"]
    )
    table = pa.Table.from_pandas(df)
...
def test_write_to_dataset_category_observed(tempdir):
    ...
    df = pd.DataFrame(
        {"cat": pd.Categorical(["a", "b", "a"], categories=["a", "b", "c"]), "col": [1, 2, 3]}
    )
    table = pa.table(df)
