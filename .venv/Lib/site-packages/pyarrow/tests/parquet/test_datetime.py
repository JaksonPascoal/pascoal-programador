...
def test_coerce_timestamps_truncated(tempdir):
    ...
    _write_table(
        table_us, filename, version="2.6", coerce_timestamps="ms", allow_truncated_timestamps=True
    )
...
def test_coerce_int96_timestamp_unit(unit):
    ...
    _check_roundtrip(
        table, expected, read_table_kwargs=read_table_kwargs, use_deprecated_int96_timestamps=True
    )
    _check_roundtrip(
        table,
        expected,
