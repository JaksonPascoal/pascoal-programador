...
def test_uniform_encrypted_parquet_write_read(tempdir, data_table):
    ...
    kms_connection_config, crypto_factory = write_encrypted_file(
        path, data_table, FOOTER_KEY_NAME, COL_KEY_NAME, FOOTER_KEY, b"", encryption_config
    )
...
def test_encrypted_parquet_write_col_key_and_uniform_encryption(tempdir, data_table):
    ...
    with pytest.raises(OSError, match=r"Cannot set both column_keys and uniform_encryption"):
        # Write with encryption properties
        write_encrypted_file(
            path, data_table, FOOTER_KEY_NAME, COL_KEY_NAME, FOOTER_KEY, b"", encryption_config
        )
...
@pytest.mark.xfail(
    reason="Plaintext footer - reading plaintext column subset" " reads encrypted columns too"
)
