...
def test_hash_join():
    ...
    result = joined.to_table()
    expected = pa.table(
        [[1, 2, 3], [4, 5, 6], [None, 2, 3], [None, 4, 5]], names=["key", "a", "key", "b"]
    )
...
def test_hash_join_with_residual_filter():
    ...
    join_opts = HashJoinNodeOptions(
        "inner", left_keys="key", right_keys="key", filter_expression=pc.equal(pc.field("a"), 5)
    )
    joined = Declaration("hashjoin", options=join_opts, inputs=[left_source, right_source])
    result = joined.to_table()
    ...
    result = joined.to_table()
    expected = pa.table(
        [[2, 1, 3], [5, 4, 6], [2, None, None], [4, None, None]], names=["key", "a", "key", "b"]
    )
    assert result.equals(expected)
...
def test_asof_join():
    ...
    result = joined.to_table()
    expected = pa.table(
        [[1, 2, 3], [1, 1, 1], [4, 5, 6], [None, 4, None]], names=["key", "ts", "a", "b"]
    )
