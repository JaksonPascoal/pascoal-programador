...
def test_array_from_scalar():
    ...
    cases = [
        ...
        (
            now_with_tz,
            2,
            pa.array([now_utc] * 2, type=pa.timestamp("us", tz=pytz.timezone("US/Eastern"))),
        ),
...
def test_string_view_from_buffers():
    array = pa.array(
        ["String longer than 12 characters", None, "short", "Length is 12"], type=pa.string_view()
    )

    buffers = array.buffers()
    copied = pa.StringViewArray.from_buffers(pa.string_view(), len(array), buffers)
    copied.validate(full=True)
    assert copied.to_pylist() == ["String longer than 12 characters", None, "short", "Length is 12"]
...
def test_list_from_arrays(list_array_type, list_type_factory):
    ...
    expected = pa.array(
        [pyvalues[:2], pyvalues[2:5], pyvalues[5:8]], type=list_type_factory(pa.binary())
    )
...
def test_half_float_array_from_python():
    ...
    assert arr.to_pylist() == [-5, 0, 1.0, 2.0, 3, None, 12344.0, 1.234375, float("inf")]
    assert str(arr) == (
        "[\n  -5,\n  0,\n  1,\n  2,\n  3,\n  null,\n  12344," "\n  1.234375,\n  inf\n]"
    )
...
def test_cast_from_null():
    ...
    out_types = [
        pa.union(
            [pa.field("a", pa.binary(10)), pa.field("b", pa.string())], mode=pa.lib.UnionMode_DENSE
        ),
        pa.union(
            [pa.field("a", pa.binary(10)), pa.field("b", pa.string())], mode=pa.lib.UnionMode_SPARSE
        ),
    ]
...
def test_time32_time64_from_integer():
    ...
    expected = pa.array(
        [datetime.time(microsecond=1), datetime.time(microsecond=2), None], type=pa.time64("us")
    )
    assert result.equals(expected)

    result = pa.array([1000, 2000, None], type=pa.time64("ns"))
    expected = pa.array(
        [datetime.time(microsecond=1), datetime.time(microsecond=2), None], type=pa.time64("ns")
    )
    assert result.equals(expected)
...
def test_array_from_naive_datetimes():
    arr = pa.array(
        [None, datetime.datetime(2017, 4, 4, 12, 11, 10), datetime.datetime(2018, 1, 1, 0, 2, 0)]
    )
...
def test_array_from_numpy_ascii(binary_type):
    ...
    expected = pa.array(["abcde", "", None, "abcde", "", None, "abcde", ""], type=expected_type)
    assert arrow_arr.equals(expected)
...
def test_total_buffer_size():
    ...
    assert a.nbytes == 21
    a = pa.array(
        [[[1, 2], [3, 4]], [[5, 6, 7], None, [8]], [[9, 10]]], type=pa.list_(pa.list_(pa.int8()))
    )
    a1 = a.slice(1, 2)
...
@pytest.mark.parametrize(
    ("offset_type", "list_type_factory"), [(pa.int32(), pa.list_), (pa.int64(), pa.large_list)]
)
def test_list_array_flatten(offset_type, list_type_factory):
    typ2 = list_type_factory(list_type_factory(pa.int64()))
    arr2 = pa.array(
        [None, [[1, None, 2], None, [3, 4]], [], [[], [5, 6], None], [[7, 8]]], type=typ2
    )
...
@pytest.mark.parametrize(
    "list_type",
    [pa.list_(pa.int32()), pa.list_(pa.int32(), list_size=2), pa.large_list(pa.int32())],
)
...
def test_array_masked():
    ...
    arr = pa.array(
        np.array([4, None, 4, 3.0], dtype="O"), mask=np.array([False, True, False, True])
    )
    assert arr.type == pa.int64()
...
def test_array_supported_masks():
    ...
    with pytest.raises(pa.ArrowTypeError):
        arr = pa.array(
            [4, None, 4, 3],
            mask=pa.array([False, True, False, True], mask=pa.array([True, True, True, True])),
        )
...
def test_c_array_protocol_device_unsupported_keyword():
    ...
    with pytest.raises(
        NotImplementedError, match=r"Received unsupported keyword argument\(s\): \['other'\]"
    ):
