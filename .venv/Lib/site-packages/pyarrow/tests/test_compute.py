...
def test_exported_functions():
    ...
    for func in functions:
        ...
        if arity is Ellipsis:
            ...
            args = [object()] * arity
        with pytest.raises(
            TypeError, match="Got unexpected argument type " "<class 'object'> for compute function"
        ):
            func(*args)
...
def test_option_class_equality(request):
    options = [
        ...
        pc.WeekOptions(
            week_starts_monday=True, count_from_zero=False, first_week_is_fully_in_year=False
        ),
...
def test_utf8_zfill():
    ...
    arr = pa.array(["1", "-2", "+3"])
    result = pc.utf8_zero_fill(arr, options=pc.ZeroFillOptions(width=5, padding="💠")).to_pylist()
    assert result == ["💠💠💠💠1", "-💠💠💠2", "+💠💠💠3"]

    # custom ASCII padding character
    arr = pa.array(["1", "-2", "+3"])
    result = pc.utf8_zero_fill(arr, options=pc.ZeroFillOptions(width=4, padding="x")).to_pylist()
...
def test_extract_regex_span():
    ...
    expected = [{"letter": [0, 1], "digit": [1, 1]}, {"letter": [1, 1], "digit": [2, 3]}]
...
def test_drop_null_table():
    ...
    expected = pa.table(
        [pa.array(["a", "d"]), pa.array(["a", "d"]), pa.array(["a", "d"])], names=["a", "b", "c"]
    )
    result = table.drop_null()
    assert result.equals(expected)
...
def _check_temporal_rounding(ts, values, unit):
    ...
    if ta.type.tz is None:
        ...
        expected = np.where(
            expected == ts, expected + pd.Timedelta(value, unit_shorthand[unit]), expected
        )
        np.testing.assert_array_equal(result, expected)
...
@pytest.mark.timezone_data
@pytest.mark.parametrize(
    "unit", ("nanosecond", "microsecond", "millisecond", "second", "minute", "hour", "day")
)
@pytest.mark.pandas
...
def test_sort_indices_table():
    ...
    assert result.to_pylist() == [1, 0, 3, 2]
    result = pc.sort_indices(
        table, sort_keys=[("a", "descending"), ("b", "ascending")], null_placement="at_start"
    )
    assert result.to_pylist() == [2, 1, 0, 3]
    # Positional `sort_keys`
    result = pc.sort_indices(
...
def test_map_lookup():
    ...
    arr = pa.array(
        [[("one", 1), ("two", 2)], [("none", 3)], [], [("one", 5), ("one", 7)], None], type=ty
    )
...
def test_case_when():
    assert pc.case_when(
        pc.make_struct([True, False, None], [False, True, None]), [1, 2, 3], [11, 12, 13]
    ) == pa.array([1, 12, None])
...
def test_random():
    ...
    with pytest.raises(
        TypeError,
        match=r"initializer should be 'system', an integer, " r"or a hashable object; got \[\]",
    ):
...
def test_rank_quantile_options():
    ...
    # Ensure sort_keys tuple usage
    result = pc.rank_quantile(arr, options=pc.RankQuantileOptions(sort_keys=[("b", "ascending")]))
    assert result.equals(expected)
...
def test_rank_normal_options():
    ...
    result = pc.rank_normal(
        arr, options=pc.RankQuantileOptions(null_placement="at_start", sort_keys="descending")
    )
    assert result.to_pylist() == expected
...
def create_sample_expressions():
    ...
    schema = pa.schema(
        [pa.field("i64", pa.int64()), pa.field("foo", pa.struct([pa.field("bar", pa.string())]))]
    )
...
def test_pivot_wider():
    ...
    result = pc.pivot_wider(["height", "width", "depth"], [10, None, 11], key_names=key_names)
    assert result.as_py() == {"width": None, "height": 10}
    ...
    with pytest.raises(ValueError, match="Encountered more than one non-null value"):
        result = pc.pivot_wider(["height", "width", "height"], [10, None, 11], key_names=key_names)
...
def test_winsorize():
    ...
    result = pc.winsorize(arr, options=pc.WinsorizeOptions(lower_limit=0.1, upper_limit=0.8))
    assert result.to_pylist() == [8, 4, 8, 8, 5, 3, 7, 2, 2, 6]
