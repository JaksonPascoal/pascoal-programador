...
def test_nested_arrays(seq, inner_seq, factory):
    arr = pa.array(seq([inner_seq([]), inner_seq([1, 2]), None]), type=factory(pa.int64()))
    assert len(arr) == 3
    assert arr.null_count == 1
...
def test_struct_from_dicts():
    ty = pa.struct(
        [pa.field("a", pa.int32()), pa.field("b", pa.string()), pa.field("c", pa.bool_())]
    )
...
def test_struct_from_list_of_pairs():
    ...
    ty = pa.struct(
        [pa.field("a", pa.int32()), pa.field("a", pa.string()), pa.field("b", pa.bool_())]
    )
    data = [
        [("a", 5), ("a", "foo"), ("b", True)],
        [("a", 6), ("a", "bar"), ("b", False)],
...
def test_struct_from_dicts_inference():
    expected_type = pa.struct(
        [pa.field("a", pa.int64()), pa.field("b", pa.string()), pa.field("c", pa.bool_())]
    )
    data = [{"a": 5, "b": "foo", "c": True}, {"a": 6, "b": "bar", "c": False}]
    ...
    expected_type = pa.struct(
        [
            pa.field(
                "a", pa.struct([pa.field("aa", pa.list_(pa.int64())), pa.field("ab", pa.bool_())])
            ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [
                pa.scalar(
                    [("a", 1), ("b", "bar")], type=pa.struct([("a", pa.int8()), ("b", pa.string())])
                )
            ],
...
def test_array_accepts_pyarrow_scalar_errors(seq):
    ...
    with pytest.raises(
        pa.ArrowInvalid,
        match="pyarrow scalars cannot be mixed with other " "Python scalar values currently",
    ):
        pa.array(sequence)
    ...
    with pytest.raises(
        pa.ArrowInvalid, match="Cannot append scalar of type string " "to builder for type int32"
    ):
        pa.array([pa.scalar("a")], type=pa.int32())

    with pytest.raises(
        pa.ArrowInvalid, match="Cannot append scalar of type int64 " "to builder for type null"
    ):
