...
def make_random_csv(num_cols=2, num_rows=10, linesep="\r\n", write_names=True):
    ...
    columns = [pa.array(row, type=pa.int64()) for row in split_columns(arr, num_cols, num_rows)]
...
def test_read_options(pickle_module):
    ...
    match = (
        "ReadOptions: autogenerate_column_names cannot be true when" " column_names are provided"
    )
    with pytest.raises(pa.ArrowInvalid, match=match):
        opts = cls()
...
class BaseTestCSV(abc.ABC):
    ...
    def test_row_number_offset_in_errors(self):
        ...
        with pytest.raises(pa.ArrowInvalid, match=message_columns):
            self.read_bytes(
                csv_bad_columns, read_options=read_options, convert_options=convert_options
            )

        csv_bad_type = csv + b"a,b,c,d\r\n"
        message_value = format_msg(
            "In CSV column #0: {}" "CSV conversion error to int32: invalid value 'a'", 102, csv
        )
        ...
        with pytest.raises(pa.ArrowInvalid, match=message_long):
            self.read_bytes(
                csv_bad_columns_long, read_options=read_options, convert_options=convert_options
            )
        ...
        csv_bad_type = csv + b"a,b,c,d\r\n"
        message_value = format_msg(
            "In CSV column #0: {}" "CSV conversion error to int32: invalid value 'a'", 101
        )
...
class BaseCSVTableRead(BaseTestCSV):
    ...
    def test_include_columns(self):
        ...
        with pytest.raises(
            KeyError, match="Column 'xx' in include_columns " "does not exist in CSV file"
        ):
    ...
    def test_simple_varied(self):
        ...
        table = self.read_bytes(rows)
        schema = pa.schema(
            [("a", pa.float64()), ("b", pa.int64()), ("c", pa.string()), ("d", pa.bool_())]
        )
        assert table.schema == schema
        assert table.to_pydict() == {
            "a": [1.0, 4.0],
    ...
    def test_custom_nulls(self):
        ...
        table = self.read_bytes(rows, convert_options=opts)
        schema = pa.schema(
            [("a", pa.null()), ("b", pa.string()), ("c", pa.string()), ("d", pa.int64())]
        )
        assert table.schema == schema
        assert table.to_pydict() == {
            "a": [None, None],
...
class BaseStreamingCSVRead(BaseTestCSV):
    ...
    def test_inference(self):
        ...
        self.check_reader(
            reader, expected_schema, [{"a": ["123", "abc", "gh"], "b": [b"456", b"de\xff", b"ij"]}]
        )
        ...
        self.check_reader(
            reader,
            expected_schema,
            [{"a": ["123", "abc"], "b": [b"456", b"de\xff"]}, {"a": ["gh"], "b": [b"ij"]}],
        )
