...
def test_context_from_object(size):
    ...
    with pytest.raises(
        pa.ArrowTypeError, match=("cannot create device buffer view from " ".* 'numpy.ndarray'")
    ):
...
def test_copy_to_host(size):
    ...
    for position, nbytes in [(0, size + 1), (size // 2, (size + 1) // 2 + 1), (size, 1)]:
        with pytest.raises(
            ValueError, match=("requested more to copy than" " available from device buffer")
        ):
            dbuf.copy_to_host(position=position, nbytes=nbytes)
...
def test_copy_from_device(dest_ctx, size):
    ...
    for position, nbytes in [(size // 2, (size + 1) // 2 + 1)]:
        with pytest.raises(
            ValueError, match=("requested more to copy than" " available in device buffer")
        ):
            put(position=position, nbytes=nbytes)
...
def test_copy_from_host(size):
    ...
    for position, nbytes in [
        ...
        with pytest.raises(
            ValueError, match=("requested more to copy than" " available from host buffer")
        ):
            put(position=position, nbytes=nbytes)
