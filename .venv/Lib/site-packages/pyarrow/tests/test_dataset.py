...
def test_filesystem_dataset(mockfs):
    ...
    fragments = [
        file_format.make_fragment(path, mockfs, part) for path, part in zip(paths, partitions)
    ]
...
def test_take(dataset, dataset_reader):
    ...
    for indices in [[1, 7], pa.array([1, 7])]:
        assert dataset_reader.take(dataset, indices) == dataset_reader.to_table(dataset).take(
            indices
        )
    with pytest.raises(IndexError):
...
def test_partitioning():
    ...
    schema = pa.schema(
        [pa.field("group", pa.int64()), pa.field("key", pa.dictionary(pa.int8(), pa.string()))]
    )
...
def test_dataset_partitioning_format(
    ...
    if flavor != "HivePartitioning":
        ...
        with pytest.raises(
            pa.ArrowInvalid,
            match=("No partition key for foo but a key was provided" " subsequently for bar"),
        ):
...
def test_fragment_scan_options_pickling(pickle_module):
    options = [
        ...
        ds.JsonFragmentScanOptions(
            pa.json.ParseOptions(newlines_in_values=False, unexpected_field_behavior="error")
        ),
        ds.JsonFragmentScanOptions(
...
def test_filesystem_factory(mockfs, paths_or_selector, pre_buffer):
    format = ds.ParquetFileFormat(
        read_options=ds.ParquetReadOptions(dictionary_columns={"str"}), pre_buffer=pre_buffer
    )
...
def test_make_fragment_with_size(s3_example_simple):
    ...
    fragments_with_size = [
        file_format.make_fragment(path, fs, file_size=size) for path, size in zip(paths, sizes_true)
    ]
...
def test_make_csv_fragment_from_buffer(dataset_reader, pickle_module):
    ...
    expected = pa.table(
        [["a", "b", "c"], [12, 11, 10], ["dog", "cat", "rabbit"]], names=["alpha", "num", "animal"]
    )
...
def test_fragments_reconstruct(tempdir, dataset_reader, pickle_module):
    ...
    pattern = r"No match for FieldRef.Name\(part\) in " + fragment.physical_schema.to_string(
        False, False, False
    )
...
def test_fragments_parquet_row_groups_predicate(tempdir):
    ...
    row_group_fragments = list(
        fragment.split_by_row_group(filter=ds.field("part") == "a", schema=dataset.schema)
    )
    assert len(row_group_fragments) == 2

    # filter contradicts partition_expression: no row groups
    row_group_fragments = list(
        fragment.split_by_row_group(filter=ds.field("part") == "b", schema=dataset.schema)
    )
...
def _create_partitioned_dataset(basedir):
    ...
    full_table = table.append_column("part", pa.array([0] * 3 + [1] * 3 + [2] * 3, type=pa.int32()))
...
def test_open_dataset_partitioned_directory(tempdir, dataset_reader, pickle_module):
    ...
    dataset = ds.dataset(
        str(path), partitioning=ds.partitioning(pa.schema([("part", pa.int8())]), flavor="hive")
    )
    expected_schema = table.schema.append(pa.field("part", pa.int8()))
    assert dataset.schema.equals(expected_schema)

    result = dataset.to_table()
    expected = table.append_column("part", pa.array([0] * 3 + [1] * 3 + [2] * 3, type=pa.int8()))
    assert result.equals(expected)
...
def test_dataset_partitioned_dictionary_type_reconstruct(tempdir, pickle_module):
    ...
    dataset = ds.dataset(
        tempdir, format="feather", partitioning=ds.HivePartitioning.discover(infer_dictionary=True)
    )
...
def test_union_dataset_from_other_datasets(tempdir, multisourcefs):
    ...
    child2 = ds.dataset(
        "/schema", filesystem=multisourcefs, format="parquet", partitioning=["week", "color"]
    )
...
def test_ipc_format(tempdir, dataset_reader):
    table = pa.table(
        {"a": pa.array([1, 2, 3], type="int8"), "b": pa.array([0.1, 0.2, 0.3], type="float64")}
    )
...
def test_orc_writer_not_implemented_for_dataset():
    with pytest.raises(
        NotImplementedError, match="Writing datasets not yet implemented for this file format"
    ):
...
def test_csv_format(tempdir, dataset_reader):
    table = pa.table(
        {"a": pa.array([1, 2, 3], type="int64"), "b": pa.array([0.1, 0.2, 0.3], type="float64")}
    )
...
def test_csv_format_options(tempdir, dataset_reader):
    ...
    dataset = ds.dataset(
        path, format=ds.CsvFileFormat(read_options=pa.csv.ReadOptions(column_names=["foo"]))
    )
    result = dataset_reader.to_table(dataset)
...
def test_csv_format_options_generate_columns(tempdir, dataset_reader):
    ...
    dataset = ds.dataset(
        path,
        format=ds.CsvFileFormat(read_options=pa.csv.ReadOptions(autogenerate_column_names=True)),
    )
    result = dataset_reader.to_table(dataset)
...
def test_json_format_options(tempdir, dataset_reader):
    ...
    with pytest.raises(ValueError, match="try to increase block size"):
        dataset = ds.dataset(
            path, format=ds.JsonFileFormat(read_options=pa.json.ReadOptions(block_size=4))
        )
...
def test_feather_format_compressed(tempdir, compression, dataset_reader):
    ...
    write_options = file_format.make_write_options(compression=compression)
    ds.write_dataset(
        table, str(basedir / "data.arrow"), format=file_format, file_options=write_options
    )
...
def test_write_to_dataset_given_null_just_works(tempdir):
    schema = pa.schema(
        [pa.field("col", pa.int64()), pa.field("part", pa.dictionary(pa.int32(), pa.string()))]
    )
...
def test_write_dataset_partitioned(tempdir):
    ...
    _check_dataset_roundtrip(
        dataset, str(target), expected_paths, "f1", target, partitioning=partitioning_schema
    )
...
def test_write_dataset_with_scanner(tempdir):
    ...
    with tempfile.TemporaryDirectory() as tempdir2:
        ds.write_dataset(
            dataset.scanner(columns=["b", "c"]), tempdir2, format="ipc", partitioning=["b"]
        )
...
def test_write_dataset_max_open_files(tempdir):
    ...
    record_batch_4 = pa.record_batch(
        data=[[13, 14, 15, 16, 0, 1], ["a", "b", "c", "d", "e", "b"]], names=column_names
    )
    ...
    ds.write_dataset(
        data=table, base_dir=data_source_1, partitioning=partitioning, format=file_format
    )
...
def test_write_dataset_use_threads_preserve_order(tempdir):
    ...
    ds.write_dataset(batches, tempdir, format="parquet", use_threads=True, preserve_order=True)
...
def test_write_scanner(tempdir, dataset_reader):
    ...
    with pytest.raises(ValueError, match="Cannot specify a schema"):
        ds.write_dataset(
            dataset_reader.scanner(dataset), base_dir, schema=table.schema, format="feather"
        )
...
def test_dataset_join_asof_collisions(tempdir):
    t1 = pa.table(
        {"colA": [1, 2, 6], "colB": [10, 20, 60], "on": [1, 2, 3], "colVals": ["a", "b", "f"]}
    )
    ds.write_dataset(t1, tempdir / "t1", format="ipc")
    ds1 = ds.dataset(tempdir / "t1", format="ipc")
...
def test_write_dataset_preserve_nullability(tempdir):
    ...
    schema_nullable = pa.schema(
        [pa.field("x", pa.int64(), nullable=False), pa.field("y", pa.int64(), nullable=True)]
    )
...
def test_write_dataset_preserve_field_metadata(tempdir):
    schema_metadata = pa.schema(
        [pa.field("x", pa.int64(), metadata={b"foo": b"bar"}), pa.field("y", pa.int64())]
    )
    ...
    ds.write_dataset(
        [table_no_meta, table], tempdir / "test3", format="parquet", schema=schema_metadata
    )
