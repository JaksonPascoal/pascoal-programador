...
def multiple_column_table():
    return pa.Table.from_arrays(
        [pa.array(["foo", "bar", "baz", "qux"]), pa.array([1, 2, 3, 4])], names=["a", "b"]
    )
...
class ConstantFlightServer(FlightServerBase):
    ...
    def list_flights(self, context, criteria):
        if criteria == self.CRITERIA:
            yield flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path("/foo"), [])
...
def test_eq():
    items = [
        ...
        lambda: (
            flight.FlightEndpoint(b"foo", [flight.Location("grpc+tcp://localhost:1234")]),
        ...
        lambda: (
            ...
            flight.FlightEndpoint(
                b"foo", [], pa.scalar("2023-04-05T12:34:56.789").cast(pa.timestamp("ms"))
            ),
        ),
        lambda: (
        ...
        lambda: (
            ...
            flight.FlightInfo(
                pa.schema([("ints", pa.int64())]), flight.FlightDescriptor.for_path(), []
            ),
        ),
        lambda: (
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path(), []),
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_command(b"foo"), []),
        ),
        lambda: (
            flight.FlightInfo(
                pa.schema([]),
        ...
        lambda: (
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path(), [], ordered=False),
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path(), [], ordered=True),
        ),
        lambda: (
            flight.FlightInfo(
            ...
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], app_metadata=b"meta"
            ),
        ),
        lambda: (
...
def test_flight_info_defaults():
    ...
    fi2 = flight.FlightInfo(
        pa.schema([]), flight.FlightDescriptor.for_path(), [], total_records=-1, total_bytes=-1
    )
    fi3 = flight.FlightInfo(
        pa.schema([]), flight.FlightDescriptor.for_path(), [], total_records=None, total_bytes=None
    )
...
def test_location_unknown_scheme():
    ...
    assert (
        flight.Location("https://example.com/bar.parquet").uri == b"https://example.com/bar.parquet"
    )
...
def test_tls_do_get():
    ...
    with (
        ConstantFlightServer(tls_certificates=certs["certificates"]) as s,
        FlightClient(("localhost", s.port), tls_root_certs=certs["root_cert"]) as client,
    ):
        data = client.do_get(flight.Ticket(b"ints")).read_all()
        assert data.equals(table)
...
def test_middleware_reject():
    ...
    with (
        ...
        client = FlightClient(
            ("localhost", server.port), middleware=[SelectiveAuthClientMiddlewareFactory()]
        )
...
def test_middleware_mapping():
    ...
    with (
        ...
        FlightClient(("localhost", server.port), middleware=[client_middleware]) as client,
    ):
...
def test_generic_options():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s:
        ...
        client = flight.connect(
            ("localhost", s.port), tls_root_certs=certs["root_cert"], generic_options=options
        )
        with pytest.raises(flight.FlightUnavailableError):
            client.do_get(flight.Ticket(b"ints"))
        client.close()
