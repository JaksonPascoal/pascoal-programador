...
def py_fsspec_s3fs(request, s3_server):
    ...
    fs = s3fs.S3FileSystem(
        key=access_key, secret=secret_key, client_kwargs=dict(endpoint_url=f"http://{host}:{port}")
    )
    fs = PyFileSystem(FSSpecHandler(fs))
    fs.create_dir(bucket)
...
@pytest.fixture(
    params=[
        ...
        pytest.param(
            "py_fsspec_localfs", id="PyFileSystem(FSSpecHandler(fsspec.LocalFileSystem()))"
        ),
        pytest.param(
            "py_fsspec_memoryfs", id='PyFileSystem(FSSpecHandler(fsspec.filesystem("memory")))'
        ),
        pytest.param(
            "py_fsspec_s3fs",
...
def test_subtree_filesystem():
    ...
    assert subfs.base_fs == localfs
    assert repr(subfs).startswith(
        "SubTreeFileSystem(base_path=/another/base/," " base_fs=<pyarrow._fs.LocalFileSystem"
    )
...
def test_azurefs_options(pickle_module):
    ...
    with pytest.raises(ValueError, match="client_id must be specified"):
        AzureFileSystem(account_name="fake-account-name", client_secret="fake-client-secret")
    ...
    with pytest.raises(ValueError, match=invalid_msg):
        AzureFileSystem(
            account_name="fake-account-name", tenant_id="fake-tenant-id", client_id="fake-client-id"
        )

    with pytest.raises(ValueError):
        AzureFileSystem(
            account_name="fake-account-name", account_key="fakeaccount", sas_token="fakesastoken"
        )
...
def test_hdfs_options(hdfs_connection, pickle_module):
    ...
    hdfs11 = HadoopFileSystem(
        host, port, user="localuser", kerb_ticket="cache_path", extra_conf={"hdfs_token": "abcd"}
    )
...
def test_copy_files(s3_connection, s3fs, tempdir):
    ...
    source_uri = (
        f"s3://{access_key}:{secret_key}@{path}" f"?scheme=http&endpoint_override={host}:{port}"
    )
