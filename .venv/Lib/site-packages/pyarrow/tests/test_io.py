...
@pytest.mark.parametrize(
    "val, expected_hex_buffer", [(b"check", b"636865636B"), (b"\a0", b"0730"), (b"", b"")]
)
...
def test_memory_map_large_seeks():
    if sys.maxsize >= 2**32:
        ...
        expected_error = pytest.raises(
            ...
            match="Requested memory map length 4294967306 " "does not fit in a C size_t",
        )
...
def test_buffered_input_stream_detach_non_seekable():
    ...
    f = pa.BufferedInputStream(
        pa.BufferedInputStream(pa.BufferReader(b"123456789"), buffer_size=4), buffer_size=4
    )
    assert f.read(2) == b"12"
    raw = f.detach()
    assert f.closed
...
def test_transcoding_input_stream(src_encoding, dest_encoding):
    ...
    # Incremental
    check_transcoding(
        unicode_transcoding_example, src_encoding, dest_encoding, itertools.cycle([1, 2, 3, 5])
    )
