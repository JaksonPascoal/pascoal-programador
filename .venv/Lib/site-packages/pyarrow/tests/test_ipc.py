    ...
    def write_batches(self, num_batches=5, as_table=False):
        ...
        for i in range(num_batches):
            batch = pa.record_batch(
                [[random.random() for _ in range(nrows)], ["foo", None, "bar", "bazbaz", "qux"]],
                schema=schema,
            )
    ...
    df = pd.DataFrame(
        {
            ...
            "two": pd.Categorical(
                ["foo", np.nan, "bar", "foo", "foo"], categories=["foo", "bar"], ordered=True
            ),
        }
    )
...
def test_batches_with_custom_metadata_roundtrip(ipc_type):
    ...
    if ipc_type == "stream":
        ...
        with pa.ipc.open_file(buffer) as reader:
            batch_with_metas = [
                reader.get_batch_with_custom_metadata(i) for i in range(reader.num_record_batches)
            ]
...
def test_ipc_stream_no_batches():
    # ARROW-2307
    table = pa.Table.from_arrays(
        [pa.array([1, 2, 3, 4]), pa.array(["foo", "bar", "baz", "qux"])], names=["a", "b"]
    )
