...
def test_get_library_dirs_win32():
    assert any(
        os.path.exists(os.path.join(directory, "arrow.lib")) for directory in pa.get_library_dirs()
    )
...
def test_set_timezone_db_path_non_windows():
    ...
    with pytest.raises(
        ArrowInvalid, match="Arrow was set to use OS timezone " "database at compile time"
    ):
