...
def check_example_file(orc_path, expected_df, need_fix=False):
    ...
    for i in range(orc_file.nstripes):
        ...
        check_example_values(
            pd.DataFrame(batch.to_pydict()), expected_df, start=json_pos, stop=json_pos + len(batch)
        )
        json_pos += len(batch)
...
def test_orcfile_empty(datadir):
    ...
    expected_schema = pa.schema(
        [
            ...
            (
                "map",
                pa.map_(pa.string(), pa.struct([("int1", pa.int32()), ("string1", pa.string())])),
            ),
        ]
    )
    ...
    fields = [
        ...
        pa.field(
            ...
            pa.list_(
                pa.field(
                    "item",
                    pa.struct([pa.field("inner1", pa.int64()), pa.field("inner2", pa.int64())]),
                )
            ),
        ),
