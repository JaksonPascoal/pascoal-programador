...
def _check_pandas_roundtrip(
    ...
    table = klass.from_pandas(
        df, schema=schema, preserve_index=preserve_index, nthreads=2 if use_threads else 1
    )
...
class TestConvertMetadata:
    ...
    @pytest.mark.filterwarnings("ignore:The DataFrame has column names of mixed type:UserWarning")
...
class TestConvertDateTimeLikeTypes:
    ...
    @pytest.mark.parametrize(
        "coerce_to_ns,expected_dtype", [(False, "datetime64[ms]"), (True, "datetime64[ns]")]
    )
    ...
    @pytest.mark.parametrize(
        "coerce_to_ns,expected_type", [(False, "datetime64[ms]"), (True, "datetime64[ns]")]
    )
    ...
    def test_date_objects_typed(self):
        ...
        ex_values = np.array(
            ["2017-04-03", "2017-04-04", "2017-04-04", "2017-04-05"], dtype="datetime64[D]"
        )
    ...
    @pytest.mark.skipif(
        np is not None and Version("1.16.0") <= Version(np.__version__) < Version("1.16.1"),
    ...
    def test_timedeltas_nulls(self, unit):
        if Version(pd.__version__) < Version("2.0.0"):
            unit = "ns"
        df = pd.DataFrame(
            {"timedelta64": np.array([0, None, 7200000000000], dtype=f"timedelta64[{unit}]")}
        )
        field = pa.field("timedelta64", pa.duration(unit))
        schema = pa.schema([field])
        _check_pandas_roundtrip(
...
class TestConvertStringLikeTypes:
    ...
    def test_pandas_unicode(self):
        ...
        field = pa.field(
            "strings", pa.large_string() if _pandas_api.uses_string_dtype() else pa.string()
        )
        schema = pa.schema([field])
...
class TestConvertListTypes:
    ...
    def test_column_of_decimal_list(self):
        ...
        expected_df = pd.DataFrame(
            {"col1": [[decimal.Decimal("1"), decimal.Decimal("2")], [decimal.Decimal("3.3")]]}
        )
        tm.assert_frame_equal(df, expected_df)
    ...
    def test_column_of_lists_chunked(self):
        # ARROW-1357
        df = pd.DataFrame(
            {"lists": np.array([[1, 2], None, [2, 3], [4, 5], [6, 7], [8, 9]], dtype=object)}
        )
    ...
    def test_nested_lists_all_none(self):
        ...
        data2 = np.array(
            [None, None, [None, None], np.array([None, None], dtype=object)], dtype=object
        )
        arr = pa.array(data2)
    ...
    def test_infer_numpy_array(self):
        data = OrderedDict(
            [("ints", [np.array([0, 1], dtype=np.int64), np.array([2, 3], dtype=np.int64)])]
        )
        df = pd.DataFrame(data)
    ...
    def test_nested_large_list(self):
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings(
                "ignore", "Creating an ndarray from ragged nested", _np_VisibleDeprecationWarning
            )
            warnings.filterwarnings("ignore", "elementwise comparison failed", DeprecationWarning)
    ...
    def test_large_binary_list(self):
        for list_type_factory in (pa.list_, pa.large_list):
            s = pa.array(
                [["aa", "bb"], None, ["cc"], []], type=list_type_factory(pa.large_binary())
            ).to_pandas()
            tm.assert_series_equal(
                s, pd.Series([[b"aa", b"bb"], None, [b"cc"], []]), check_names=False
            )
            s = pa.array(
                [["aa", "bb"], None, ["cc"], []], type=list_type_factory(pa.large_string())
            ).to_pandas()
            tm.assert_series_equal(
    ...
    def test_map_array_with_nulls(self):
        ...
        expected = [
            [(k, float(v) if v is not None else None) for k, v in row] if row is not None else None
            for row in data
        ]
    ...
    def test_list_view_to_pandas_multiple_chunks(self, klass):
        ...
        arr1 = klass.from_arrays(
            offsets=pa.array([2, 1, 0]), sizes=pa.array([2, 2, 2]), values=pa.array([1, 2, 3, 4])
        )
...
class TestConvertStructTypes:
    ...
    def test_from_tuples(self):
        ...
        _check_pandas_roundtrip(
            df, expected=expected_df, schema=expected_schema, expected_schema=expected_schema
        )
...
class TestConvertMisc:
    ...
    def test_strided_data_import(self):
        ...
        # strings
        cases.append(
            np.array([random_ascii(10) for i in range(N * K)], dtype=object).reshape(N, K).copy()
        )
        ...
        cases.append(
            np.arange("2016-01-01T00:00:00.001", N * K, dtype="datetime64[ms]").reshape(N, K).copy()
        )
    ...
    def test_partial_schema(self):
        ...
        _check_pandas_roundtrip(
            df, schema=partial_schema, expected=df[["c", "a"]], expected_schema=partial_schema
        )
...
def test_to_pandas_deduplicate_strings_array_types():
    if _pandas_api.uses_string_dtype():
        pytest.skip("pandas uses string dtype and not object dtype, keyword has no effect")
    nunique = 100
    repeats = 10
...
def test_to_pandas_deduplicate_date_time():
    ...
    for raw_type, array_type, pandas_options in cases:
        ...
        _assert_nunique(
            casted_arr.to_pandas(deduplicate_objects=False, **pandas_options), len(casted_arr)
        )
...
def test_table_from_pandas_checks_field_nullability():
    ...
    schema = pa.schema(
        [pa.field("a", pa.float64(), nullable=False), pa.field("b", pa.utf8(), nullable=False)]
    )
...
def test_table_from_pandas_keeps_column_order_of_dataframe():
    ...
    schema2 = pa.schema(
        [("floats", pa.float64()), ("partition", pa.int64()), ("arrays", pa.list_(pa.int64()))]
    )
...
def test_table_from_pandas_keeps_column_order_of_schema():
    ...
    schema = pa.schema(
        [("floats", pa.float64()), ("arrays", pa.list_(pa.int32())), ("partition", pa.int32())]
    )
...
def test_table_from_pandas_columns_argument_only_does_filtering():
    ...
    schema1 = pa.schema(
        [("arrays", pa.list_(pa.int64())), ("floats", pa.float64()), ("partition", pa.int64())]
    )
...
def test_table_from_pandas_schema_index_columns():
    ...
    with pytest.raises(
        ValueError, match="name 'index' is present in the " "schema, but it is a RangeIndex"
    ):
    ...
    _check_pandas_roundtrip(
        df, schema=schema, preserve_index=None, expected_schema=schema, expected=expected
    )
    _check_pandas_roundtrip(
        df, schema=schema, preserve_index=True, expected_schema=schema, expected=expected
    )
...
def test_array_uses_memory_pool():
    ...
    N = 10000
    arr = pa.array(
        np.arange(N, dtype=np.int64), mask=np.random.randint(0, 2, size=N).astype(np.bool_)
    )
...
def test_variable_dictionary_to_pandas():
    ...
    i4 = pa.array(
        np.random.randint(0, len(d3), size=1000, dtype="i4"), mask=np.random.rand(1000) < 0.1
    )
...
def test_array_protocol_pandas_extension_types(monkeypatch):
    ...
    monkeypatch.setattr(
        pd.arrays.PeriodArray, "__arrow_array__", PandasArray__arrow_array__, raising=False
    )
    monkeypatch.setattr(
        pd.arrays.IntervalArray, "__arrow_array__", PandasArray__arrow_array__, raising=False
    )
    for arr in [
            ...
            b"pandas": json.dumps(
                {
                    ...
                    "index_columns": [
                        {"kind": "range", "name": "qux", "start": 0, "step": 2, "stop": 8}
                    ],
