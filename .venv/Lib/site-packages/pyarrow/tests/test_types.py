...
def get_many_types():
    ...
    return (
        ...
        pa.struct(
            [pa.field("a", pa.int32()), pa.field("b", pa.int8()), pa.field("c", pa.string())]
        ),
        pa.struct(
            [
...
def test_is_map():
    ...
    fields = pa.map_(
        pa.field("key_name", pa.utf8(), nullable=False), pa.field("value_name", pa.int32())
    )
...
def test_convert_custom_tzinfo_objects_to_string():
    ...
    msg = (
        r"Object returned by tzinfo.utcoffset\(None\) is not an instance " r"of datetime.timedelta"
    )
