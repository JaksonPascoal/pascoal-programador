...
def nullary_func_fixture():
    ...
    def nullary_func(context):
        return pa.array(
            [42] * context.batch_length, type=pa.int64(), memory_pool=context.memory_pool
        )
...
def test_udf_array_binary(binary_func_fixture):
    check_scalar_function(
        binary_func_fixture, [pa.array([10, 20], pa.int64()), pa.array([2, 4], pa.int64())]
    )
...
def test_hash_agg_basic(unary_agg_func_fixture):
    ...
    expected = (
        table.group_by("id").aggregate([("value", "mean")]).rename_columns(["id", "value_mean_udf"])
    )
...
def test_hash_agg_empty(unary_agg_func_fixture):
    ...
    result = table.group_by("id").aggregate([("value", "mean_udf")])
    expected = pa.table(
        [pa.array([], pa.int32()), pa.array([], pa.float64())], names=["id", "value_mean_udf"]
    )
...
def test_hash_agg_random(sum_agg_func_fixture):
    ...
    expected = (
        table.group_by("id").aggregate([("value", "sum")]).rename_columns(["id", "value_sum_udf"])
    )
