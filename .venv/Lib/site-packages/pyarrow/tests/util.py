...
def randdecimal(precision, scale):
    ...
    if scale < 0:
        raise ValueError(
            "randdecimal does not yet support generating decimals with " "negative scale"
        )
...
def _wait_for_minio_startup(mcdir, address, access_key, secret_key):
    ...
    while time.time() - start < 10:
        try:
            _run_mc_command(
                mcdir, "alias", "set", "myminio", f"http://{address}", access_key, secret_key
            )
            return
...
def _configure_s3_limited_user(s3_server, policy, username, password):
    ...
    try:
        ...
        _run_mc_command(
            mcdir, "admin", "policy", "create", "myminio/", "no-create-buckets", policy_path
        )
        _run_mc_command(
            mcdir, "admin", "policy", "attach", "myminio/", "no-create-buckets", "--user", username
        )
